...
void loop() {
  ...
  if (((data[5] & 0xC0) >> 6) == 1) {
    pre = goal;
    goal = gyro_x - goaltg * 40;
    int prego = gyro_x - goaltg;
    if (prego > 358 || prego < 2 )goal = 0;
    vt = goal;
    ...
    vt = vt + map(testch[0] - 6, 364, 1684, -3000, 3000);
    int ctl1 = map(testch[0] - 6, 364, 1684, -3000, 3000);
    if (abs(vt) < 5)vt = 0;
    if (ctl1 > 5 || ctl1 < -5) {
      goaltg = gyro_x;
    }

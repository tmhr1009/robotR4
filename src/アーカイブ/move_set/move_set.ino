...
void loop() {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    Serial.print(i);
    Serial.print("] : ");
    Serial.println(m3508_1.u[i]);
  }

  for (int i = 0; i < msg.len; i++) {
    msg.buf[i * 2] = m3508_1.u[i] >> 8;
    msg.buf[i * 2 + 1] = m3508_1.u[i] & 0xFF;
  }

  for (int i = 0; i < 4; i++)m3508_1.u[i] = 0;
  delay(5);
}
...
void timerInt() {
  ...
  while ( CANTransmitter.read(rxmsg) ) {
    //    m3508_1.update();
    if (rxmsg.id == 0x201) {
      int pid0up = rxmsg.buf[2] * 256 + rxmsg.buf[3];
      m3508_1.update_pid0(pid0up);
    }
    if (rxmsg.id == 0x202) {
      int pid1up = rxmsg.buf[2] * 256 + rxmsg.buf[3];
      m3508_1.update_pid1(pid1up);
    }
    if (rxmsg.id == 0x203) {
      int pid2up = rxmsg.buf[2] * 256 + rxmsg.buf[3];
      m3508_1.update_pid2(pid2up);
    }
  }
}
